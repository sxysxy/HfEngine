...
class Generator
	...
	def method_missing(method_name, *arg, &block)
		...
		if @native_object.respond_to? method_name
			...
			raise Exception, "\n\t#{e.backtrace}\nHFSF Compile Error in #{method_name.to_s} #{arg.to_s}\n" + e.message
		end
	...
	def compile(context)
		...
		@tobe_compiled.each do |emm|
			if !x[:compiled][emm[0].to_sym]  #Avoid re-compiling
				x[:compiled][emm[0].to_sym] = emm[1].new.from_string(@code, emm[0]).byte_code
	...
	sdata[:sets].each {|set|
		...
		when SectionGenerator::CBUFFER_INFO 
			if set.cbuffer.nil? #set to nullptr(example : set_ps_cbuffer(nil, 0), this will clear the cbuffer on slot 0
				...
				raise GeneratingLogicError, "#{set.cbuffer} is not a HEG::ConstantBuffer" if !cb.is_a?(HEG::ConstantBuffer)
